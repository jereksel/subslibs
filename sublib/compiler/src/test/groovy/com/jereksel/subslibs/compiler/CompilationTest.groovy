package com.jereksel.subslibs.compiler

import com.jereksel.subslibs.colorreader.Color
import com.jereksel.subslibs.colorreader.ColorReader
import com.jereksel.subslibs.compilercommon.ThemeToCompile
import com.jereksel.subslibs.compilercommon.Type1DataToCompile
import com.jereksel.subslibs.models.Type1Extension
import com.jereksel.subslibs.reader.ThemeReader
...
class CompilationTest extends Specification {
    ...
    def "Color from apk reading test"() {
        ...
        then:
        getColorsValues(apk).iterator().toList() == [new Color("my_color", "0xffabcdef")]
    }
    ...
    def "When color is available it's value is returned"() {
        ...
        when:
        def color = getColorValue(apk, "my_color")
    ...
    def "When color doesn't exist exception is throws"() {
        ...
        when:
        getColorValue(apk, "color_that_doesnt_exist")
    ...
    def "Compiled theme should have all colors"() {
        when:
        def file = compileTheme(compilationDir: new File(resources, "basicTheme"))
        def colors = getColorsValues(file)
    ...
    def "Type1 type1.xml should be replaced"() {
        ...
        def apk = compileTheme(compilationDir: compilationDir, type1: types1)
        def colors = getColorsValues(apk)
    ...
    def getColorsValues(apk) {
        return ColorReader.getColorsValues(new File(aapt.aaptPath), apk)
    }

    def getColorValue(apk, color) {
        return ColorReader.getColorValue(new File(aapt.aaptPath), apk, color)
    }
